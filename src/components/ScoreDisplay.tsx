...
import { midiToStaffPosition, needsLedgerLines, getAccidentalForPitch } from '../utils/musicNotation';
...
export const ScoreDisplay: React.FC<ScoreDisplayProps> = ({
  ...
  const calculateScrollSpeed = useCallback((notes: Note[], canvasWidth: number) => {
    ...
    const rightmostX = notes.reduce((max, note) => 
      Math.max(max, note.xPosition), 0
    );
  ...
  const drawStaff = (ctx: CanvasRenderingContext2D, y: number, width: number) => {
    ctx.strokeStyle = '#000';
    ctx.lineWidth = 1;
  ...
  const drawClef = (ctx: CanvasRenderingContext2D, type: 'treble' | 'bass', y: number) => {
    ctx.fillStyle = '#000';
    ctx.font = '48px serif';
  ...
  const drawNote = (ctx: CanvasRenderingContext2D, note: Note, x: number) => {
    ...
    if (note.midiPitch === 0) {
      ...
      // Draw note head
      drawNoteHead(ctx, x, noteY, note.noteType);
      
      // Draw stem for quarter notes and shorter
      if (note.noteType >= 4) {
        drawStem(ctx, x, noteY, note.noteType);
      }
    }
  };

  const drawNoteHead = (ctx: CanvasRenderingContext2D, x: number, y: number, noteType: number) => {
    ctx.fillStyle = '#000';
    ctx.strokeStyle = '#000';
    ctx.lineWidth = 1.5;
  ...
  const drawStem = (ctx: CanvasRenderingContext2D, x: number, y: number, noteType: number) => {
    ctx.strokeStyle = '#000';
    ctx.lineWidth = 1.5;
    ...
    if (noteType === 8) {
      drawFlag(ctx, stemX, y - stemHeight, 1);
    } else if (noteType === 16) {
      drawFlag(ctx, stemX, y - stemHeight, 2);
    }
  };

  const drawFlag = (ctx: CanvasRenderingContext2D, x: number, y: number, count: number) => {
    ctx.strokeStyle = '#000';
    ctx.lineWidth = 1.5;
  ...
  const drawRest = (ctx: CanvasRenderingContext2D, noteType: number, x: number, y: number) => {
    ctx.fillStyle = '#000';
    ctx.font = '20px serif';
  ...
  const animate = useCallback((currentTime: number) => {
    ...
    const updatedNotes = notes
      .map(note => ({
        ...
        barlineX: note.barlineX !== undefined ? note.barlineX - scrollSpeed * deltaTime : undefined,
      }))
